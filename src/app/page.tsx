...
const ForestUI = () => {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    if (now - lastRippleTime.current > RIPPLE_INTERVAL_MS) {
      const rippleId = now;
      setRipples((prev) => [...prev.slice(-MAX_ELEMENTS + 1), { id: rippleId, x: xPercent, y: yPercent }]);
      setTimeout(() => {
    ...
    if (now - lastSpawnTime.current > SPAWN_INTERVAL_MS && cursorPos.current) {
      ...
      const id = now;
      setTrails((prev) => [...prev.slice(-MAX_ELEMENTS + 1), { id, x: xPercent, y: yPercent }]);
      setTimeout(() => {
  ...
  return (
      ...
      >
        ...
        <p className="text-center text-lg mb-6 text-gray-700">Move your mouse to discover hidden forest magic!</p>
        ...
        {ripples.map((r) => (
            <div
                ...
                style={{ top: `${r.y}%`, left: `${r.x}%`, transform: 'translate(-50%, -50%)' }}
            />
        ))}
