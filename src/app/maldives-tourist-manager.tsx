...
export default function IslandQuest() {
  ...
  return (
    ...
    >
      ...
      <section className="container mx-auto px-4 py-8 flex-1">
          ...
          {filteredExperiences.map((experience) => (
            ...
            >
              ...
              <div className="p-5">
                ...
                <button
                  className="px-4 py-2 bg-cyan-500 hover:bg-cyan-600 text-white rounded-full transition w-full cursor-pointer"
        ...
        {showBooked && bookingExp && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -40 }}
            className={`fixed top-4 right-4 ${darkMode ? "bg-slate-800 text-white border-cyan-700" : "bg-white text-slate-900 border-cyan-300"} shadow-lg rounded-full px-4 py-2.5 flex items-center gap-2 z-50 border text-sm`}
      ...
      <footer
        className={`${darkMode ? "bg-slate-900 text-slate-400" : "bg-white text-slate-500"} text-center py-4 px-4 text-xs mt-8 border-t border-slate-200`}
      >
        <div>
          © {new Date().getFullYear()} IslandQuest. All rights reserved.
        </div>
        <div>
          Experience data, photos, and booking features are for UI demo purpose
          only.
        </div>
      </footer>
    </div>
