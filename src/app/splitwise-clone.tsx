...
const SplitwiseClone = () => {
  ...
  const validateForm = (): boolean => {
    ...
    const hasEmptySplit = Object.values(split).some(
    ...
    if (hasEmptySplit) {
      ...
      totalAmount !== "" &&
      Math.abs(splitSum - Number(totalAmount)) > 0.001
    ) {
      errors.split =
  ...
  const evenSplit = useMemo(() => {
    ...
    if (Math.abs(diff) > 0.001 && people.length > 0) {
      ...
      newSplit[firstPerson] = parseFloat(
        (Number(newSplit[firstPerson]) + diff).toFixed(2),
      );
    }
    return newSplit;
  }, [totalAmount]);
  ...
  const calculateEvenSplit = () => {
    if (!evenSplit) {
      setSplitEvenlyError(
        "Please enter a valid total amount before splitting.",
      );
      return;
    }
    setSplitEvenlyError("");
                    ...
                    <div key={person}>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          value={split[person]}
                          onChange={(e) =>
                            handleSplitChange(person, e.target.value)
                          }
                          placeholder="0.00"
                          min="0"
                          step="0.01"
              ...
              <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                {expenses.map((expense) => (
                  <li key={expense.id} className="py-4">
                    ...
                    <div
                      className={`p-3 rounded-lg text-sm ${getClassName(darkMode, "bg-gray-700", "bg-gray-100")}`}
                    >
