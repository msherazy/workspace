...
import { IoEllipse, IoSettings, IoSquare, IoTrash, IoTriangle, IoChevronDown, IoChevronUp, IoChevronForward } from "react-icons/io5";
...
const ModernPanel = ({ children, title, lightTheme }: { children: ReactNode, title: string, lightTheme: boolean }) => (
    <motion.div
    ...
    >
      {title && (
          <h3 className={`text-xl font-bold mb-4 tracking-tight ${lightTheme ? "text-gray-800" : "text-white"}`}>
            {title}
          </h3>
      )}
      {children}
...
const CollapsiblePanel = ({ title, children, defaultOpen = true, lightTheme }: {
  title: string,
  children: ReactNode,
  defaultOpen?: boolean,
  lightTheme: boolean
  ...
  return (
      <div className="mb-4">
        ...
        >
          <span className={`font-medium ${lightTheme ? "text-gray-800" : "text-white"}`}>{title}</span>
          <motion.div
              animate={{ rotate: isOpen ? 180 : 0 }}
              transition={{ duration: 0.2 }}
          >
            {isOpen ? (
                <IoChevronUp className={`h-5 w-5 ${lightTheme ? "text-gray-600" : "text-gray-300"}`} />
            ) : (
                <IoChevronDown className={`h-5 w-5 ${lightTheme ? "text-gray-600" : "text-gray-300"}`} />
            )}
          </motion.div>
        </button>
      ...
      <div className={`min-h-screen flex flex-col transition-colors duration-300 ${lightTheme ? "bg-gradient-to-br from-gray-50 to-gray-100" : "bg-gradient-to-br from-gray-900 to-gray-800"}`}>
        ...
        <main className="flex flex-col flex-1 p-4 gap-4">
          <div className="flex flex-col md:flex-row gap-4 h-full">
            ...
            >
              <ModernPanel lightTheme={lightTheme} title={isToolsCollapsed ? "" : "Shape Editor"}>
                ...
                >
                  {isToolsCollapsed ? (
                      <IoChevronForward className={`h-5 w-5 ${lightTheme ? "text-gray-600" : "text-gray-300"}`} />
                  ) : (
                      <IoChevronUp className={`h-5 w-5 ${lightTheme ? "text-gray-600" : "text-gray-300"}`} />
                  )}
                </button>
                ...
                {!isToolsCollapsed && (
                    <>
                      <CollapsiblePanel title="Create New Shape" lightTheme={lightTheme}>
