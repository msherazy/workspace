...
const Dashboard = () => {
  ...
  const summaryMetrics = useMemo(() => {
    const totalYield = cropData.reduce(
      (sum, week) => sum + (Number(week[selectedCrop]) || 0),
      0,
    );
    const totalWater = cropData.reduce(
      (sum, week) => sum + (Number(week.water) || 0),
      0,
    );
    const totalFertilizer = cropData.reduce(
      (sum, week) => sum + (Number(week.fertilizer) || 0),
      0,
    );
    ...
    return (
      ...
      >
        <div
          role="status"
          aria-live="polite"
          className={`flex flex-col items-center ${theme.text.primary}`}
        >
          <svg
  ...
  if (error) {
    return (
      <div
        className={`min-h-screen flex items-center justify-center p-4 ${theme.background}`}
      >
        <div
          role="alert"
          aria-live="assertive"
          className="p-6 bg-red-100 border-l-4 border-red-500 text-red-700 rounded-md max-w-md w-full"
        >
  ...
  return (
    <div
      className={`${theme.background} min-h-screen font-sans ${theme.transition}`}
    >
      {/* Header */}
      <header
        className={`py-8 px-6 mb-10 border-b border-slate-300 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-center ${theme.transition}`}
      >
        <div>
      ...
      <main className="container mx-auto px-4" aria-label="Dashboard Content">
        ...
        >
          <div className="sr-only" id="performance-summary-heading">
            Performance Summary
          </div>
          <div
          ...
          >
            ...
            <div>
              <h2
                id="water-usage-summary-heading"
                className={`text-sm ${theme.text.tertiary}`}
              >
                Total Water Usage
              </h2>
          ...
          >
            ...
            <div>
              <h2
                id="fertilizer-usage-summary-heading"
                className={`text-sm ${theme.text.tertiary}`}
              >
              ...
              <p className={`text-2xl font-semibold ${theme.text.primary}`}>
                {new Intl.NumberFormat("en-US").format(
                  summaryMetrics.totalFertilizer,
                )}
              </p>
            </div>
          </div>
        ...
        <section className="mb-10" aria-labelledby="crop-output-heading">
          <div className="flex flex-col sm:flex-row items-center justify-between mb-4">
            ...
            <div className="mt-4 sm:mt-0 flex items-center">
              <label
                htmlFor="crop-select"
                className={`mr-2 ${theme.text.secondary}`}
              >
                Select Crop:
              </label>
              <select
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                <XAxis
                  dataKey="name"
                  stroke={theme.chart.stroke}
                  tick={{ fill: theme.chart.stroke }}
                />
                <YAxis
                  stroke={theme.chart.stroke}
                  tick={{ fill: theme.chart.stroke }}
            ...
            <div className="sr-only" aria-live="polite">
              ...
              {cropData.map(
                (week) => `${week.name}: ${week[selectedCrop]} units. `,
              )}
            </div>
          </div>
        </section>
        ...
        <section className="mb-10" aria-labelledby="resource-usage-heading">
          <h2
            ...
            className={`text-3xl font-bold mb-6 ${theme.text.primary}`}
          >
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            >
              <h3
                id="water-usage-heading"
                className={`text-2xl font-semibold mb-2 ${theme.text.primary}`}
              >
                Water Usage
              </h3>
              <div className="w-full h-60" aria-label="Water usage chart">
                ...
                <div className="sr-only" aria-live="polite">
                  ...
                  {cropData.map(
                    (week) => `${week.name}: ${week.water} units. `,
                  )}
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div className="w-full h-60" aria-label="Fertilizer usage chart">
                ...
                <div className="sr-only" aria-live="polite">
                  ...
                  {cropData.map(
                    (week) => `${week.name}: ${week.fertilizer} units. `,
                  )}
                </div>
              </div>
            </div>
      ...
      {/* Footer */}
      <footer
        className={`py-6 text-center text-sm ${theme.transition} ${theme.text.tertiary} border-t border-slate-200 dark:border-slate-800`}
        role="contentinfo"
      >
        <div className="container mx-auto px-4">
          <p>
            &copy; {new Date().getFullYear()} Farm Dashboard. All rights
            reserved.
          </p>
